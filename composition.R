...
fig_2 <- ggplot(data = U_CA_3_1_size, aes(x = size)) +
...
ggsave(here("plots", "fig_2.tiff"), plot = fig_2)  


# absorbance spectra of the purified samples
fig_3a <- ggplot(data = ocean_abs %>% 
...
fig_5a <- ggplot(data = ocean_fluoro %>% 
                   filter(type == "urea" & sample == "purified") %>%
...
fig_5b <- ggplot(data = ocean_fluoro %>% 
                   filter(type == "thiourea" & sample == "purified") %>%
                   filter(wavelength > 400)) +
  geom_line(aes(x = wavelength, 
                y = intensity, 
...
fig_5c <- ggplot(data = ocean_fluoro %>% 
                   filter(type == "urea" & composition == "1:1") %>%
                   filter(wavelength > 400)) +
  geom_line(aes(x = wavelength, 
                y = intensity, 
  ...
  theme(legend.position = c(0.85, 0.7)) +
  scale_color_brewer(palette = "Dark2")
ggsave(here("plots", "fig_5c.tiff"), plot = fig_5c)  

fig_5d <- ggplot(data = ocean_fluoro %>% 
                   filter(type == "thiourea" & composition == "1:1") %>%
                   filter(wavelength > 400)) +
  ...
  theme(legend.position = c(0.85, 0.7)) +
  scale_color_brewer(palette = "Dark2")
ggsave(here("plots", "fig_5d.tiff"), plot = fig_5d)  


# emission at different dilutions urea:CA 1:1
fig_6 <- ggplot(data = plate %>%
                  filter(plate == 2) %>%
...
ggsave(here("plots", "fig_6.tiff"), plot = fig_6)
...
fig_8 <- ggplot(data = thiourea_maps) +
  geom_contour_filled(aes(x = var_wavelength, y = fixed_wavelength, z = intensity), bins = 20) +
  labs(x = "Emission, nm", y = "Excitation, nm") +
  facet_wrap(~factor(composition, 
...
fig_9 <- ggplot(data = urea_hg_maps) +
  geom_contour_filled(aes(x = var_wavelength, y = fixed_wavelength, z = intensity), bins = 20) +
  labs(x = "Emission, nm", y = "Excitation, nm") +
...
fig_11 <- ggplot(data = thiourea_hg_maps) +
  geom_contour_filled(aes(x = var_wavelength, y = fixed_wavelength, z = intensity), bins = 20) +
  labs(x = "Emission, nm", y = "Excitation, nm") +
  facet_grid(composition ~ hg) +
  theme(legend.position = "none")
ggsave(here("plots", "fig_11.tiff"), plot = fig_11)
...
hg_maps <- bind_rows(urea_hg_maps, thiourea_hg_maps)


# hg_rel <- hg_maps %>%
#   group_by(type, composition, fixed_wavelength, var_wavelength) %>%
#   summarise(hg = 10,
#             intensity = ifelse(min(intensity) > 0, max(intensity) / min(intensity), NA)) %>%
#   group_by(type, composition) %>%
#   mutate(intensity = intensity / max(intensity))

#hg_maps <- bind_rows(hg_maps, hg_rel)

  
ggplot(data = hg_rel) +
  geom_contour_filled(aes(x = var_wavelength, y = fixed_wavelength, z = intensity), bins = 20) +
  labs(x = "Emission, nm", y = "Excitation, nm") +
  facet_grid(factor(paste(type, composition)) ~ factor(hg)) + #, 
  #levels = c("7_1", "5_1", "3_1", "1_1", "1_3",  "1_5", "1_7"),
  #labels = c("7:1", "5:1", "3:1", "1:1", "1:3",  "1:5", "1:7")), 
  #ncol = 4) +
  theme(legend.position = "none")
...
# temp4 <- urea_fluoro %>%
#   filter(cell == "G07" & exp_type == "emission")


# ggplot(data = urea_fluoro %>%
#          filter(exp_type == "emission" & fixed_wavelength == 365), aes(x = var_wavelength, y = intensity)) +
#      geom_line(aes(color = cell, group = id))
# 
# 
# ggplot(data = urea_fluoro %>%
#          filter(cell == "G07" & exp_type == "emission")) +
#   geom_contour_filled(aes(x = var_wavelength, y = fixed_wavelength, z = intensity)) +
#   labs(x = "Emission, nm", y = "Excitation, nm")



# temp5 <- temp3 %>%
#   filter(sample %in% c("Un0001 (A01)", "Un0067 (G07)", "Un0079 (F11)", "Un0070 (G10)") & exp_type == "emission")


# ggplot(data = urea_fluoro %>%
#          filter(cell %in% c("A01", "G07", "F11", "G10") & exp_type == "emission")) +
#   geom_contour_filled(aes(x = var_wavelength, y = fixed_wavelength, z = intensity)) +
#   labs(x = "Emission, nm", y = "Excitation, nm") +
...
ggplot(data = urea_Hg_fluoro %>%
         filter(cell %in% c("B01", "B02", "C03", "C04", "C07", "C08", "F11", "F12") & exp_type == "emission")) +
  geom_contour_filled(aes(x = var_wavelength, y = fixed_wavelength, z = intensity)) +
  labs(x = "Emission, nm", y = "Excitation, nm") +
  facet_wrap(~factor(cell, levels = c("B01", "C03", "C07", "F11", "B02",  "C04", "C08", "F12")), ncol = 4) +
  theme(legend.position = "bottom")

emission_Hg <- ggplot(data = urea_Hg_fluoro %>%
         filter(cell %in% c("A01", "A02", "B03", "B04", "C05", "C06", "E07", "E08", "G09", "G10", "A11", "A12", "F11", "F12") & exp_type == "emission")) +
  geom_contour_filled(aes(x = var_wavelength, y = fixed_wavelength, z = intensity)) +
  labs(x = "Emission, nm", y = "Excitation, nm") +
  facet_wrap(~factor(cell, levels = c("A01", "B03", "C05", "E07", "G09",  "A11", "F11", "A02", "B04", "C06", "E08", "G10", "A12", "F12")), ncol = 7) +
  theme(legend.position = "bottom")


# ggplot(data = urea_Hg_fluoro %>%
#          filter(Hg == 0 & exp_type == "emission" & composition == "1_3")) +
#   geom_contour_filled(aes(x = var_wavelength, y = fixed_wavelength, z = intensity)) +
#   labs(x = "Emission, nm", y = "Excitation, nm") +
#   facet_wrap(~factor(dilution), ncol = 4) +
#   theme(legend.position = "bottom")


excitation_Hg <- ggplot(data = urea_Hg_fluoro %>%
         filter(cell %in% c("A01", "A02", "B03", "B04", "C05", "C06", "E07", "E08", "G09", "G10", "A11", "A12", "F11", "F12") & exp_type == "excitation")) +
  geom_contour_filled(aes(x = var_wavelength, y = fixed_wavelength, z = intensity)) +
  labs(x = "Excitation, nm", y = "Emission, nm") +
  facet_wrap(~factor(cell, levels = c("A01", "B03", "C05", "E07", "G09",  "A11", "F11", "A02", "B04", "C06", "E08", "G10", "A12", "F12")), ncol = 7) +
  theme(legend.position = "none")

emission_Hg / excitation_Hg

temp <- urea_Hg_fluoro %>%
  filter(!is.na(composition)) %>%
  pivot_wider(names_from = Hg,
              values_from = intensity,
              names_prefix = "Hg",
              id_cols = c(fixed_wavelength, exp_type, type, composition, dilution, var_wavelength)) %>%
  mutate(Hg0 = Hg0 - min(Hg0),
         Hg1 = Hg1 - min(Hg1),
         rel_intensity = abs(Hg0 / Hg1))
...
ggplot(data = temp %>%
         filter(exp_type == "emission") %>%
         filter(composition == "7_1") %>%
         filter(var_wavelength <= 700)) +
  geom_contour_filled(aes(x = var_wavelength, y = fixed_wavelength, z = rel_intensity)) +
  labs(x = "Emission, nm", y = "Excitation, nm") +
  facet_wrap(~factor(dilution), ncol = 4) +
  theme(legend.position = "bottom")
...
temp2 <- temp %>% 
  filter(exp_type == "emission") %>%
  group_by(composition, dilution) %>% 
  filter(rel_intensity == max(rel_intensity))

ggplot(data = temp2) +
  geom_line(aes(x = dilution, y = rel_intensity, color = composition))

ggplot(data = urea_Hg_fluoro %>%
         filter(description == "water" & exp_type == "emission" & intensity < 0)) +
  geom_contour_filled(aes(x = var_wavelength, y = fixed_wavelength, z = intensity)) +
  labs(x = "Emission, nm", y = "Excitation, nm") +
  facet_wrap(~factor(cell), ncol = 7) +
  theme(legend.position = "bottom")


ggplot(data = urea_Hg_fluoro %>% filter(exp_type == "emission" & var_wavelength > 700)) +
  geom_histogram(aes(x = intensity), binwidth = 5)
...
ggplot(data = plate %>%
         ...
         filter(exp_type == "emission") %>%
         filter(fixed_wavelength != 380) %>%
         filter(composition == "1_7" & type == "urea") %>%
         filter(var_wavelength <= 700)) +
  geom_contour_filled(aes(x = var_wavelength, y = fixed_wavelength, z = intensity)) +
  labs(x = "Emission, nm", y = "Excitation, nm") +
  facet_wrap(~factor(cell), ncol = 4) +
  theme(legend.position = "bottom")
...
ggplot(data = plate %>%
         filter(plate == 3) %>%
         filter(exp_type == "emission") %>%
         filter(fixed_wavelength == 540) %>%
         filter(composition == "7_1" & type == "urea") %>%
         filter(var_wavelength <= 700)) +
  geom_line(aes(x = var_wavelength, y = intensity, color = cell)) +
  labs(x = "Emission, nm", y = "Intensity") +
  theme(legend.position = "right")


ggplot(data = ocean_fluoro %>% 
         filter(type == "urea" & sample == "dialysate") %>%
         filter(wavelength > 400)) +
  geom_line(aes(x = wavelength, 
                y = intensity, 
                color = composition), size = 2) +
  labs(x = "Wavelength, nm", y = "Intensity", color = "Sample") +
  theme(legend.position = c(0.85, 0.7)) +
  scale_color_brewer(palette = "Dark2")
...
detection_wl_tmp <- data.frame(composition = c("7_1", "7_1", "7_1"),
                           fixed_wavelength = c(540, 440, 360),
                           var_wavelength = c(630, 550, 460))
...
ggplot(data = detection_tmp, aes(x = hg_conc, y = rel_intensity, color = factor(fixed_wavelength))) +
  geom_point(size = 3) +
  geom_smooth(se = F, method = 'loess', size = 2) +
  scale_x_log10(limits = c(2e-8, 2e-4)) +
  theme(legend.position = c(0.9, 0.8)) +
...
detection_wl_tmp <- data.frame(composition = c("1_1", "1_1"),
                               fixed_wavelength = c(540, 460),
                               var_wavelength = c(630, 550))
